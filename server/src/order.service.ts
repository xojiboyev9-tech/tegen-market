...
export class OrderService {
  ...
  async create(userId: number, data: any) {
    ...
    if (!items.length) {
      throw new BadRequestException('Buyurtmada mahsulotlar mavjud emas');
    }
      ...
      if (cashback > 0) {
        await this.prisma.$transaction(async (tx) => {
          await tx.order.update({
            ...
            data: {
              status,
              approvedById: approvedById ?? order.approvedById,
          ...
          await tx.cashbackTransaction.create({
            data: {
              ...
              amount: cashback,
              type: 'EARNED',
              description: `Buyurtma #${order.id} uchun 2% cashback`,
            },
          });
        });
      } else {
